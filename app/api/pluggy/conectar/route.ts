 ...
 *   - Usuario com plano = 'max' (Open Finance e exclusivo do Max)
 */
export async function POST(request: Request) {
  try {
    ...
    // 2) Confirma plano Max (Open Finance e exclusivo do Max)
    ...
    if (perfil.plano !== 'max') {
      return NextResponse.json(
        { erro: 'Open Finance esta disponivel apenas para o plano Max.' },
        { status: 403 }
      );
    }
